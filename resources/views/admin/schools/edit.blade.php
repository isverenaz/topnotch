                        ...
                        <div class="panel-body">
                            ...
                            <div class="tab-content p-3 text-muted">
                                        ...
                                        <div class="tab-pane @if(++$key ==1) active @endif" id="{{$lang['code']}}"
                                            ...
                                            <div class="row g-3">
                                                <div class="col-12">
                                                    ...
                                                    <input type="text" class="form-control" name="name[{{$lang['code']}}]" value="{{ !empty($school['name'][$lang['code']])? $school['name'][$lang['code']]: NULL }}">
                                                </div>
                                                <div class="col-12">
                                                    <label class="form-label">@lang('admin.text') - {{$lang['code']}}</label>
                                                    <textarea class="form-control" type="text" name="text[{{$lang['code']}}]" >{{ !empty($school['text'][$lang['code']])? $school['text'][$lang['code']]: NULL }}</textarea>
                                                </div>
                                                <div class="col-12">
                                                    ...
                                                    <textarea class="editor form-control"
                                                              ...
                                                              name="full_text[{{$lang['code']}}]">{!! !empty($school['full_text'][$lang['code']])? $school['full_text'][$lang['code']]: NULL !!}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                <div class="tab-pane" id="other" role="tabpanel">
                                    <div class="row g-3">
                                        <div class="col-md-12">
                                            <label for="category_id" class="form-label">@lang('admin.categories')</label>
                                            <select class="form-control" name="category_id" id="category_id">
                                                <option value="">@lang('admin.choose')</option>
                                                @foreach($categories as $category)
                                                    <option value="{{$category->id}}" @if($school->category_id == $category->id) selected="selected" @endif>{{ !empty($category['title'][$currentLang])? $category['title'][$currentLang]: null }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-12">
                                            <label for="country_id" class="form-label">@lang('admin.countries')</label>
                                            <select class="form-control" name="country_id" id="country_id">
                                                <option value="">@lang('admin.choose')</option>
                                                @foreach($countries as $country)
                                                    <option value="{{$country->id}}" @if($school->country_id == $country->id) selected="selected" @endif>{{ !empty($country['name'][$currentLang])? $country['name'][$currentLang]: null }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-12">
                                            ...
                                            <select class="form-control" name="teacher_id" id="teacher_id">
                                                ...
                                                @foreach($teachers as $teacher)
                                                    <option value="{{$teacher->id}}" @if($school->teacher_id == $teacher->id) selected="selected" @endif>{{ !empty($teacher['name'][$currentLang])? $teacher['name'][$currentLang]: null }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-12">
                                            ...
                                            <select class="form-control" name="language_id" id="language_id">
                                                ...
                                                @foreach($mainLanguages as $language)
                                                    <option value="{{$language->id}}" @if($school->language_id == $language->id) selected="selected" @endif>{{ !empty($language['name'][$currentLang])? $language['name'][$currentLang]: null }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-12" id="university-wrapper">
                                            ...
                                            <select class="form-control" name="parent_language_id" id="parent_language_id">
                                                <option value="{{$school->parent_language_id}}">{{$school['parentLanguage']['name'][$currentLang]}}</option>
                                            </select>
                                        </div>
                                        <div class="col-sm-4">
                                            <label class="form-label">@lang('admin.status')</label>
                                            <select class="form-control" name="status">
                                                <option value="1" @if($school['status'] ==1) selected @endif>@lang('admin.active')</option>
                                                <option value="0" @if($school['status'] ==0) selected @endif>@lang('admin.nonactive')</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-control" name="is_main">
                                                <option value="1" @if($school['is_main'] ==1) selected @endif>@lang('admin.show')</option>
                                                <option value="0" @if($school['is_main'] ==0) selected @endif>@lang('admin.nonshow')</option>
                                            </select>
                                        </div>
                                        <div class="col-sm-4">
                                            <label class="form-label">@lang('admin.datetime')</label>
                                            <input type="text" class="form-control" name="datetime" value="{{ date('Y-m-d H:i', strtotime($school->datetime)) }}">
                                        </div>
                                                    ...
                                                    <div class="card-body">
                                                        <div class="row">
                                                            <div class="col-xxl-9 col-sm-8">
                                                                ...
                                                                <div class="col-md-5">
                                                                    <img src="{{ asset('uploads/schools/'.$school->image) }}" style="width: 288px;!important;">
                                                                </div>
                                                            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#language_id').on('change', function () {
                ...
                $.ajax({
                    url: '{{ route('admin.schools.getParentLanguagesByLanguageId') }}',
                    type: 'GET',
