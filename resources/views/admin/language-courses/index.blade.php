...
@section('admin.content')
    ...
    <div class="main-content">
        <div class="dashboard-breadcrumb mb-25">
            <h2>@lang('admin.language_courses')</h2>
            @can('language-courses-create')
            <a class="btn btn-sm btn-primary" href="{{ route('admin.language-courses.create') }}">
            ...
            <div class="col-12">
                <div class="panel">
                    <div class="panel-body">
                        <div class="row">
                            @if(!empty($languageCourses[0]) && isset($languageCourses[0]))
                                @foreach($languageCourses as $data)
                                    <div class="col-sm-6">
                                        <div class="card">
                                            <div class="card-header">
                                                {{ !empty($data['name'][$currentLang])? $data['name'][$currentLang]: null }}
                                                @can('language-courses-edit')
                                                <a href="{{ route('admin.language-courses.edit',$data['id']) }}" class="btn btn-sm btn-icon btn-primary" title="@lang('admin.edit')">
                                            ...
                                            <div class="card-body animation-card">
                                                <div class="text-center" data-aos="flip-left">
                                                    <img src="{{ asset('uploads/languageCourses/'.$data->image) }}" alt="{{ !empty($data['name']['az'])? $data['name']['az']: $data['name']['az'] }}" style="max-height: 327px;!important;">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
{{--        {{ $news->appends(request()->except('page'))->links('components.admin.pagination',['posts' => $news]) }}--}}
    ...
    <!-- main content end -->
    @if(!empty($languageCourses[0]) && isset($languageCourses[0]))
        @foreach($languageCourses as $value)
            <div class="modal fade" id="delete{{$value['id']}}" tabindex="-1" aria-labelledby="deletecategory{{$value['id']}}Label" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('admin.language-courses.destroy',$value['id']) }}" method="POST">
                            @csrf
                            @method('DELETE')
