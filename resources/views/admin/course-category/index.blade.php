            ...
            <div class="col-12">
                <div class="panel">
                    <div class="panel-header">
                        <h5>@lang('admin.course_categories')</h5>
                        <div class="btn-box d-flex flex-wrap gap-2">
                            <div id="tableSearch"></div>
                            @can('language-courses-create')
                            ...
                            @if(!empty($categories[0]) && isset($categories[0]))
                                @foreach($categories as $data)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-box">
                                                @can('language-courses-edit')
                                                    <button class="btn btn-sm btn-icon btn-primary" data-bs-toggle="modal" data-bs-target="#editMain{{ $data['id'] }}">
                                                        <i class="fa-light fa-edit"></i>
                                                    </button>
                                                @endcan
                                                @can('language-courses-delete')
    ...
    @if(!empty($categories[0]) && isset($categories[0]))
        @foreach($categories as $value)
            <div class="modal fade" id="editMain{{ $value['id'] }}" tabindex="-1" aria-labelledby="editMain{{ $value['id'] }}Label" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        <form action="{{ route('admin.course-categories.update', $value['id']) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="modal-body">
                                <ul class="nav nav-pills nav-justified" role="tablist">
                                    ...
                                    @endif
                                    <li class="nav-item waves-effect waves-light">
                                        <a class="nav-link" data-bs-toggle="tab" href="#editother-main{{ $value['id'] }}" role="tab">
                                            <span class="d-none d-sm-block">@lang('admin.other')</span>
                                        </a>
                                    </li>
                                </ul>
                                <div class="tab-content p-3 text-muted">
                                    @if(!empty($locales))
                                        @foreach($locales as $key => $lang)
                                            <div class="tab-pane @if(++$key ==1) active @endif" id="edit-main{{ $value['id'] }}{{ $lang['code'] }}" role="tabpanel">
                                                <div class="row g-3">
                                                    <div class="col-12">
                                                        <label class="form-label">@lang('admin.title') - {{ $lang['code'] }}</label>
                                    ...
                                    <div class="tab-pane" id="editother-main{{ $value['id'] }}" role="tabpanel">
                                        <div class="row g-3">
                                            <div class="col-sm-12">
                                                <label class="form-label">@lang('admin.status')</label>
                                                <select class="form-control" name="status">
                                                    <option value="1" @if($value['status'] == 1) selected @endif>@lang('admin.show')</option>
                                                    <option value="0" @if($value['status'] == 0) selected @endif>@lang('admin.nonshow')</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="modal fade" id="deleteMain{{ $value['id'] }}" tabindex="-1" aria-labelledby="deleteMain{{ $value['id'] }}Label" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('admin.course-categories.destroy', $value['id']) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <div class="modal-body">
